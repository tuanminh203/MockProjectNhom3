...
// ==========================================================
// Các API Đơn hàng (được bảo vệ bởi token)
// ==========================================================   

// Hàm đặt món từ giỏ hàng
export const placeOrderFromCart = async () => {
    try {
        const response = await api.post('/orders');
        return response.data; 
    } catch (error) {
        console.error("Lỗi khi đặt món:", error.response || error);
        throw error;
    }
};
...
export const getOrderDetails = async (orderId) => {
  try {
    ...
    console.error("Lỗi khi lấy chi tiết đơn hàng:", error.response || error);
    throw error;
  }
};
